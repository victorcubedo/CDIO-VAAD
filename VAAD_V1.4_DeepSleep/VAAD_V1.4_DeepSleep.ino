...
void loop()
{
  /*---------------------------------------------------------------------------------------------
     ...
     ---------------------------------------------------------------------------------------------*/
  //SALINIDAD
  const int sinSal = 4100 ;  // Medimos valor sin sal
  const int conSal = 2900;  // Medimos valor con sal

  //HUMEDAD
  const int AirValue = 21200;  // Medimos valor en seco
  ...
  //FUNCIÓN DORMIR
  const double tiempoDormidoS = 5.0;  //Constantes para el DeepSleep
  const double tiempoDespiertoS = 3.0;
  ...
  cont = dormir(tiempoDormidoS, tiempoDespiertoS, cont);
...
double dormir(double tiempoDormidoS, double tiempoDespiertoS, double contador)
{
  double tiempoDormidoMs = tiempoDormidoS * 1000;
  double tiempoDespiertoMs = tiempoDespiertoS * 1000;
  
  if(contador<tiempoDormidoMs)
  {
    contador = contador + 1000.0;
    delay(tiempoDespiertoMs);
    return contador;
  }
  else
  {
    ...
    Serial.println("ESP8266 está dormido...zZz");
    delay(tiempoDormidoMs);

    Serial.println("\n \n //--------------// \n");

    Serial.println("ESP8266 se ha despertado!");
    delay(1000);
    return 0.0;
  }
}
